...
//   generic_json_callbacks.js

//   Taken from Mason Core Demo files

//   This is the Generic JSON Prediction specific set of callbacks for the Mason Viewer
...
//		var rowItem = precomputeParams.rowItem;   				//  The rowItem for this row
		...
		var tooltipFromJSON = blockDataItem.tooltip;
...
//		var type = blockDataItem.type;
		...
		if ( tooltipFromJSON && tooltipFromJSON !== "" ) {
			
			tooltipHTML += "<br>" + tooltipFromJSON;
		}
...
//		var callbackDataStorage = getColorForBlockParams.callbackDataStorage;
		...
		var rowItem = getColorForBlockParams.rowItem;
		var rowData = rowItem.rowData;
		
		var forHiddenBlocks = getColorForBlockParams.forHiddenBlocks;
		
		if ( forHiddenBlocks ) {
			
			var rowXColor = rowData.rowXColor;
			
			if ( rowXColor && rowXColor !== "" ) {
...
//		var callbackDataStorage = fcnParams.callbackDataStorage;  //  Can store information here and it will be returned the next time
		...
		if ( blockDataItem.link && blockDataItem.link !== "" ) {

//			alert("Click on block received: blockDataItem.link: " + blockDataItem.link );
			
			window.location.href = blockDataItem.link;
		}
	...
	var masonViewer_Generic_JSON_Pre_Built_CallbacksCreator = function( param ){
	...
	window.masonViewer_Generic_JSON_Pre_Built_CallbacksCreator = masonViewer_Generic_JSON_Pre_Built_CallbacksCreator;
